 ...
 * Ne lance PAS le subprocess Python (ça c'est /api/metadata/start).
 *
 * Body FormData :
...
export async function POST(req: NextRequest) {
  ...
  const runId = existingRunId || `metadata_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`
  ...
  console.log(`[metadata:${runId}] chunk +${savedCount} fichiers → total ${totalSaved} sur disque`)
